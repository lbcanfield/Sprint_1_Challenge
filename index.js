...
function getArtistByIndex(tArray, tIndex)
{
  return "the artist at index " + tIndex + " is " + tArray[tIndex].name ;
}
...
function listOfNames(tArray)
{
  const tempArray = [];
  for ( let i = 0; i < tArray.length; i += 1)
  {
    tempArray.push(tArray[i].name);
  }
  return tempArray;
}
...
function removeArtist(tArray, tIndex) {
  const tempArray = [];
  for ( let i = 0; i < tArray.length; i += 1)
  {
    ...
    if (i === tIndex)
    {
      console.log("Artist Found - Skipping");
    }
    else
    {
      tempArray.push(tArray[i]);
    }
  }
  return tempArray;
}

const t5Array = removeArtist(artists, 0);
...
function addArtist(tArray, v, w, x, y, z) 
{
  const tempArray = tArray;
  const newArtist =
  {
    name: v,
    years: w,
...
function lotsOfArt(tArray) 
{
  const tempArray = []
  for ( let i = 0; i < tArray.length; i += 1)
  {
    if ( tArray[i].paintings >= 100 )
    {
      tempArray.push( tArray[i].name );
    }
  }
...
const t7Array = lotsOfArt(artists);
...
function artistInfo(tArray, tName)
{
  for ( let i = 0; i < tArray.length; i += 1)
  {
...
function artistByCountry(tArray, tNationality)
{
  const tempArray = [];
  for ( let i = 0; i < tArray.length; i += 1)
  {
    if ( tArray[i].nationality === tNationality)
    {
      tempArray.push(tArray[i].name);
    }
  }
